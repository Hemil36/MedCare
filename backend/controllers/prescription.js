...
function generatePDF({patientName, doctorName, patientEmail, prescriptionDate, diagnosis, medicationPrescription, remarks,hash}) {
    return new Promise((resolve, reject) => {
        ...
        doc.font('Helvetica-Bold').fontSize(25).fillColor('#33fab2').text('MedCare').moveDown(1);
        ...
        // Diagnosis
        doc.fontSize(14).text('Diagnosis: ' + diagnosis, { underline: true }).moveDown().fillColor('white');
        ...
        doc.rect(startX, startY, tableWidth, rowHeight).fillAndStroke('#131619', '#48d77c');
        ...
        medicationPrescription.forEach((prescription, index) => {
            ...
            doc.text(prescription.name, startX + 10, contentStartY + 10).fillColor('white')
        ...
        // Remarks Section
        doc.moveDown(2).fontSize(14).text('Remarks:', 50, doc.y, { underline: true })
            .fontSize(12).text(remarks)
            .moveDown().fillColor('white');

        // Footer
        doc.moveDown(2).fontSize(10)
            .text('This is a digitally generated prescription.', { align: 'center', oblique: true })
            .moveDown()
...
export async function sendEmail({patientName, doctorName, patientEmail, prescriptionDate, diagnosis, medicationPrescription, remarks }) {
    // Create Nodemailer transporter

    const hash = generateHash({patientEmail, prescriptionDate, diagnosis, medicationPrescription, remarks});
    generatePDF({patientName, doctorName, patientEmail, prescriptionDate, diagnosis, medicationPrescription, remarks,hash}).then(pdfBuffer => {
